...
const { BadRequestError, NotFoundError, UnauthorizedError } = require('../middleware/errorTypes');
...
exports.login = async (req, res,next) => {
    ...
    try {
        ...
        res.status(200).json({isSuccess:true, message: 'Login successful',token: token });
    } catch (error) {
        next(error);
    }
};
